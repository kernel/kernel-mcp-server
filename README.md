...
- `manage_vault_items` - List, get, invoke advertised operations (including fill with value-free bindings), observe events, and delete vault items. Read credential definitions, presence, version, and collection links without stored values. `collect` reopens the full form; provider approvals remain user actions. Ready is not login or payment success.
