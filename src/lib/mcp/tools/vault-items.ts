...
export function registerVaultItemTools(
  ...
  server.registerTool(
    ...
    {
      description:
        'Inspect credential and payment vault items and immutable audit events. "list" reads items without renewing collection links; "get" reads state, safe field metadata, version, required user actions, available_operations, and available_expansions. MCP returns explicitly non-sensitive text/email values; sensitive values and TOTP seeds are omitted. For credentials, present the collection URL only to the intended user, outside the agent-controlled browser; never ask for passwords or TOTP seeds in chat. Reopen collection using its advertised operation when available; TOTP has no hosted input. wait observes readiness, not edits to ready credentials: compare versions using get without wait. Use manage_vault_credentials for credential creation and updates; use a per-user vault, site-name-only description, and sensitive:false for ordinary usernames/emails. Never store credit card data in credential items. "invoke" fetches the item again and submits only an advertised operation; read its description and obtain explicit user approval first. Provider actions (OAuth, enrollment, MFA, approval) must be completed by the user, not invoked as operations. Link card approval starts when the card is created: give the user item.action.url; there is no authorize operation. "events" observes outcomes; use the last event ID as after. "delete" invalidates an item credential; confirm with the user first. Unresolved payments can block item and parent deletion; the API decides whether explicit abandonment is allowed, and deletion never proves a payment did not occur. recovery_required is not decline or expiry: stop payment attempts and reconcile with the provider or support; no reset exists. Credential ready means required values exist, not that login succeeded; payment ready does not mean paid. For fill, read the item\'s advertised fill description and supply exactly the inputs it names: browser_id (session ID) and usually page_url. Credentials and virtual-card fills also need ordered field/selector bindings; a Link Pay Token fill needs no fields. Values stay server-side until entering the browser. Link cards use the advertised fill operation, not aliases or egress substitution: page_url must be the exact checkout page the card was created for, and the browser must retain its vault attachment. Fill never submits payment or clicks buttons; click Pay separately, only with the user\'s authorization. Fill returns no card or token values but does not isolate them from browser/CDP access; failed or unknown fills may leave partial changes. Never automatically retry or fall back to aliases. AgentCard aliases and checkout hold/approval/replay remain supported. Follow each advertised operation\'s API contract for inputs and outcome handling; never substitute another operation or retry an uncertain attempt. Requests are never automatically retried. Do not retry failed, timed-out, rejected, or indeterminate payments; inspect state/events instead.',
      inputSchema: vaultToolInput({
        ...
        inputs: z
          ...
          .optional()
          .describe(
            "(invoke) Optional operation-specific request body fields. Read the advertised operation's description for required inputs; for fill, include fields only when it asks for field bindings. Do not include type or id_or_name; the tool sets those. Never supply secret values in chat.",
          ),
        expand: z
