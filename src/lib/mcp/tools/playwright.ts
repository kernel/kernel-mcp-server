  ...
  server.tool(
    ...
    "Execute Playwright/TypeScript automation code against an existing Kernel browser session. Does not create or delete browsers -- use manage_browsers to manage session lifecycle.",
    {
      code: z
        .string()
        .describe(
          'Playwright/TypeScript code with a `page` object in scope. Example: "await page.goto(\\"https://example.com\\"); return await page.title();" Return only what you need: prefer a targeted selector (e.g. `await page.locator(SELECTOR).innerText()`) over reading a whole page. Whole-page dumps like `innerText()`/`ariaSnapshot()` on `body` return large output that is truncated. To understand an unfamiliar page, scope the snapshot to a region (e.g. `await page.locator("main").ariaSnapshot()`).',
        ),
      session_id: z
        .string()
