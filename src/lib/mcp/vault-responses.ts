...
const onePasswordAccountGuidance = [
  ...
  'Observe with get until state.status is connected, then create 1Password credentials with manage_vault_credentials, provider: "1password", and account_id set to this item\'s id. declined or reconnect_required need the user to connect again. Invoke 1pw_recover only when advertised and after explicit user approval; never delete the account to recover.',
];

const onePasswordCredentialGuidance = [
  "1Password credentials hold no values in Kernel. After explicit user approval, invoke the advertised 1pw_request_access with inputs {browser_id} and optional goal, reason, and keywords, using a browser created with this vault attached.",
  "Approval is a human action in the account owner's 1Password app. MCP withholds the native approval link and access-request references; never open, approve, or relay an approval yourself. Tell the owner a request is waiting in 1Password, then invoke the advertised 1pw_poll_access with {browser_id} to observe the decision. Do not issue a second request while one is pending.",
  "When ready, invoke the advertised 1pw_fill with {browser_id, page_url}, where page_url is the exact current top-level URL on the requested login origin. The extension selects fields and submits; you cannot supply selectors or values. fill_submitted does not confirm login. fill_unknown may have submitted; never retry it in the same browser. 1pw_reconcile_access only abandons an unconfirmed request after the user checks 1Password for an existing one, requires acknowledge_unconfirmed: true, and can lead to duplicate requests.",
];
