...
// V3: Bordered container with a 2px kernel-green accent line on top
// Combines containment of V1 with brand color accent for more visual weight
export default function SelectOrgV3(): React.ReactElement {
  return (
    <Col className="min-h-screen items-center justify-center">
      <Col className="max-w-md w-full mx-auto p-8 gap-8">
        ...
        <div className="border-[0.5px] border-foreground border-t-2 border-t-primary">
          {MOCK_ORGS.map((org) => (
            <button
              key={org.id}
